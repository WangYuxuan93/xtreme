...
def squad_convert_example_to_features(example, max_seq_length, doc_stride, max_query_length, 
          is_training, lang2id, threshold=0.5, get_external_mention_boundary=False):
  features = []
...
def tagme_mention_boundary(tokens, max_seq_length=384, language="en", threshold=0.2): 
  ...
  #exit()
  #mention_boundaries_tensor = torch.from_numpy(np.array(mention_boundaries)).long()
  return mb_label
...
def squad_convert_examples_to_features(
  ...
  lang2id=None, threshold=0.5, get_external_mention_boundary=False
):
  """
  ...
  with Pool(threads, initializer=squad_convert_example_to_features_init, initargs=(tokenizer,)) as p:
    annotate_ = partial(
      ...
      lang2id=lang2id,
      threshold=threshold,
      get_external_mention_boundary=get_external_mention_boundary,
    )
    features = list(
      tqdm(
  ...
  add_mention_boundary = get_external_mention_boundary and examples[0].language in ["en", "de", "it"]
  if return_dataset == "pt":
    ...
    if add_mention_boundary:
      all_mention_boundaries = torch.tensor([f.mention_boundaries for f in features], dtype=torch.long)
    ...
    if not is_training:
      ...
      if add_mention_boundary:
        dataset = TensorDataset(
          ...
          all_p_mask, all_langs, all_mention_boundaries,
        )
      else:
        dataset = TensorDataset(
          all_input_ids, all_attention_masks, all_token_type_ids, all_example_index, all_cls_index, 
...
class SquadFeatures(object):
  ...
  def __init__(
    ...
    self.mention_boundaries = mention_boundaries
