...
""" Finetuning multi-lingual models on RELX (Bert, XLM, XLMRoberta)."""
...
from transformers import (
  ...
  XLMRobertaConfig,
  XLMRobertaTokenizer,
  XLMRobertaForRelationExtraction,
  MEAEConfig,
  MEAEForRelationExtraction,
  get_linear_schedule_with_warmup,
)

from processors.relx import RelxProcessor, convert_examples_to_features
...
MODEL_CLASSES = {
  "bert": (BertConfig, BertForRelationExtraction, BertTokenizer),
  "xlmr": (XLMRobertaConfig, XLMRobertaForRelationExtraction, XLMRobertaTokenizer),
  "meae": (MEAEConfig, MEAEForRelationExtraction, XLMRobertaTokenizer),
}
  ...
  for _ in train_iterator:
    ...
    for step, batch in enumerate(epoch_iterator):
      ...
      inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3],
                "ent1_ids": batch[4], "ent2_ids": batch[5]}
...
def load_and_cache_examples(args, task, tokenizer, split='train', language='en', lang2id=None, evaluate=False):
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    label_list = processor.get_labels(os.path.join(args.data_dir, "train-en.txt"))
    if split == 'train':
  ...
  if args.model_type == 'xlm':
    ...
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_ent1_ids, all_ent2_ids, all_langs)
  else:  
    dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_ent1_ids, all_ent2_ids)
  return dataset
...
def main():
  ...
  # Add special tokens for identifying entities
  special_tokens_to_add = {"additional_special_tokens": ["<e1>","</e1>","<e2>","</e2>"]}
  ...
  if args.do_train:
    ...
    # Resize token embeddings since 4 special tokens are added
